...
#%%
times, cum_P_LH = [], []
pulses = range(55778, 55795)
for pulse in pulses:
    # plasma duration
    ip, t_ip = get_sig(pulse, signals['Ip'])
    times.append(t_ip[ip > 50][-1])
    # energie
    P_LH, t_LH = get_sig(pulse, signals['LH_P_tot'])
    cum_P_LH.append(np.trapz(P_LH, t_LH))
#%%
df = pd.DataFrame(data={'duration':times, 'energy':cum_P_LH}, index=pulses)
